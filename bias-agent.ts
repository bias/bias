import { AgentDefinition } from '@codebuff/agent-sdk';

/**
 * BIAS Agent - Basic Institutional and Academic Stringency
 * Autonomous Truth Protocol for detecting and countering institutional bias
 */
export const biasAgent: AgentDefinition = {
  id: 'bias-agent',
  version: '1.8.1',
  publisher: 'om-design', // Update this to your Codebuff username/org
  
  name: 'BIAS Protocol Agent',
  description: 'Applies rigorous, recursive analysis to detect institutional and academic bias in knowledge claims. Surfaces suppressed evidence, identifies funding influences, and maps institutional incentive structures.',
  
  tags: ['analysis', 'bias-detection', 'truth-protocol', 'institutional-analysis'],
  
  inputSchema: {
    type: 'object',
    properties: {
      outputFormat: {
        description: 'Output format: text (default), json, or hybrid',
        type: 'string',
        enum: ['text', 'json', 'hybrid'],
        default: 'text'
      }
    }
  },
...
**For structured data exchange, if requested via outputFormat parameter:**
...
- **text:** Standard BIAS analysis block format (default)
- **json:** Machine-readable structured output only
